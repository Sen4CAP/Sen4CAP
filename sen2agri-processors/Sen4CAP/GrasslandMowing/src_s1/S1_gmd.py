...
# ONLY FOR SAR
def read_file_list(file_list, get_par_from_file, keys, list_orbits, polType, dataType):
    ...
    # filter wrt orbit, polarization and data type
    indx = []
    for i, p in enumerate(par_list):
        if (p['orbit'] in list_orbits) and (p['pol'] in polType) and (p['data_type'] in dataType):
            indx.append(i)
