...
void
PersistentStreamingHistogramImageFilter<TLabelImage>
::SetIntervalsLimits(const IntervalLimitsType &intervalsLimits)
{
  m_IntervalsLimits = intervalsLimits;
}
...
::ThreadedGenerateData(const RegionType& outputRegionForThread, itk::ThreadIdType threadId )
{
  ...
  for (labelIt.GoToBegin();
    ...
    {
      label = labelIt.Get();
      for (intervalIdx = 0; intervalIdx<m_IntervalsLimits.size(); intervalIdx++) {
          if (label <= m_IntervalsLimits[intervalIdx]) {
            break;
          }
      }

      // Update the accumulator
      if (m_AccumulatorMaps[threadId].count(intervalIdx) <= 0) //add new element to the map
        {
        AccumulatorType newAcc;
        m_AccumulatorMaps[threadId][intervalIdx] = newAcc;
        }
      else
        {
        m_AccumulatorMaps[threadId][intervalIdx].Update();
        }
    }
}
