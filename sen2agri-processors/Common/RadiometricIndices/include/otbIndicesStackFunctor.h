 ...
 * \ingroup OTBIndices
 */
template <typename TInput, typename TOutput, typename TIndice, typename TInput2>
class IndicesStackFunctor
{
public:
  ...
  using MaskPixelType  = typename IndiceType::MaskPixelType;
  ...
  void SetIndices(const std::vector<IndiceType*>& indices) 
  {
      if (indices.empty())
      {
   ...
   * the indices values
   */
  OutputType operator()(const PixelType& in) const
  {
    size_t idx = 0;
    OutputType out(OutputSize());
   ...
   * the indices values
   */
  OutputType operator()(const PixelType& in, const MaskPixelType& msk) const
  {
    size_t idx = 0;
    OutputType out(OutputSize());
    for (auto indice : m_Indices)
    {
      out[idx] = (*indice)(in, msk);
      ++idx;
    }
    return out;
  }
