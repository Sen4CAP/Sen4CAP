...
public:
   ...
   */
  RadiometricIndex(const std::set<BandNameType>& requiredBands) : m_bHasNoData(false), m_NoDataValue(-10000),
      m_RequiredBands(), m_BandIndices()
  {
  ...
  /**
   * Method which will compute the radiometric indice using a mask. If not overridden, the default
   * implementation uses the no mask operator()
   * \param input A itk::VariableLengthVector<TInput> holding the
   * pixel values for each band
   * \return The indice value as TOutput  (starts at 1 for first band)
   */
  virtual TOutput operator()(const itk::VariableLengthVector<TInput>& input, const itk::VariableLengthVector<TInput2>&) const
  {
      return (*this)(input);
  }
...
protected:
  ...
  {
      if (m_bHasNoData && (std::fabs(value - m_NoDataValue) < Epsilon))
      {
        return true;
      }
      return false;
