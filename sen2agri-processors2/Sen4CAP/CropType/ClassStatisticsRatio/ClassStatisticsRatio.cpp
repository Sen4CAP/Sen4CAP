...
namespace Wrapper
{
class ClassStatisticsRatio : public Application
{
public:
    typedef ClassStatisticsRatio Self;
    typedef Application Superclass;
    typedef itk::SmartPointer<Self> Pointer;
    typedef itk::SmartPointer<const Self> ConstPointer;

    itkNewMacro(Self);
    itkTypeMacro(ClassStatisticsRatio, otb::Application);
    ...
    typedef otb::RatioFilterVector<InputImageType, FeatureImageType>                   RatioFilterType;

private:
    void DoInit() override
    {
        SetName("ClassStatisticsRatio");
        ...
        AddParameter(ParameterType_Group, "in", "Input images");
        SetParameterDescription("in","This group of parameters allows one to define the input images.");

        AddParameter(ParameterType_InputImage, "in.vv", "Input co-polarisation image");
        SetParameterDescription("in.vv", "The input co-polarisation image.");

        AddParameter(ParameterType_InputImage, "in.vh", "Input cross-polarisation image");
        SetParameterDescription("in.vh", "The input cross-polarisation image.");
        ...
        SetDocExampleParameterValue("in.vv", "vv.tif");
        SetDocExampleParameterValue("in.vh", "vh.tif");
    ...
    {
        auto inputImageVV = GetParameterFloatImage("in.vv");
        auto inputImageVH = GetParameterFloatImage("in.vh");
        auto classImage = GetParameterInt32Image("ref");

        m_ImageList = ImageListType::New();
        m_ImageList->PushBack(inputImageVV);
        m_ImageList->PushBack(inputImageVH);
        ...
        m_StatisticsFilter->SetInput(m_RatioFilter->GetOutput());
        m_StatisticsFilter->SetInputLabelImage(classImage);
        m_StatisticsFilter->SetNoDataValue(0);
...
OTB_APPLICATION_EXPORT(otb::Wrapper::ClassStatisticsRatio)
