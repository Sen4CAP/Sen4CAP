...
def drop_table(conn, name):
    with conn.cursor() as cursor:
        query = SQL(""" drop table if exists {} """).format(Identifier(name))
...
def import_agricultural_practices(config, conn, pg_path, file, site_id):
    table_name = "pd_ap_staging_practices_{}_{}".format(
        site_id, config.practice.lower()
    )
    drop_table(conn, table_name)
    command = get_import_export_table_command(
        config.ogr2ogr_path,
        pg_path,
        file,
        "-nln",
    ...
    with conn.cursor() as cursor:
        query = SQL(
            ...
            Literal(site_id),
            Literal(config.practice),
            Literal(config.year),
            Literal(config.country),
        )
        print(query.as_string(conn))
        cursor.execute(query)
        conn.commit()
...
def main():
    parser = argparse.ArgumentParser(
        description="Handles the upload of the agricultural practices input tables file"
    )
    parser.add_argument(
        "-c",
        "--config-file",
    ...
    parser.add_argument(
        "-i", "--input-file", required=True, help="The uploaded input file"
    )
    parser.add_argument(
