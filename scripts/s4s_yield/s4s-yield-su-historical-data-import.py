...
def copyConfigFile(config, conn, site_short_name, config_file):
    cfgFile = getSiteConfigKey(conn, SU_YIELD_HIST_DATA_CFG_KEY, config.site_id)
    if cfgFile == "":
        print("The {} key is not configured in the database. Creating it ...".format(SU_YIELD_HIST_DATA_CFG_KEY))
        cfgFile = DEFAULT_SU_YIELD_HIST_DATA_FILE
        setConfigValue(conn, None, SU_YIELD_HIST_DATA_CFG_KEY, cfgFile,)

    cfgFile = cfgFile.replace("{site}", site_short_name)
    os.makedirs(os.path.dirname(cfgFile), exist_ok=True)
    print("Copying the SU Yield Historical Data file from {} to {}".format(config_file, cfgFile))
