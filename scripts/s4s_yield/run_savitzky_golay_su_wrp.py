...
def main():
    ...
    parser.add_argument("-y", "--year", help="The processing years", required=True, nargs="+", type=int)
    ...
    output_sg_files_dict = dict()
    output_indices_files_dict = dict()
    output_metrics_files_dict = dict()
    ...
    for year in args.year:
        ...
        for crop_type in input_files.keys() : 
            ...
            metrics_output = metrics_output + "_" + str(year) + "_" + str(crop_type) + ".csv"
        ...
        output_sg_files_dict[str(year)] = output_ct_sg_files_dict
        output_indices_files_dict[str(year)] = output_ct_indices_files_dict
        output_metrics_files_dict[str(year)] = output_ct_metrics_files_dict
    
    with open(args.sg_output, 'w') as out_sg:  
        writer = csv.writer(out_sg)
        writer.writerow(["year", "crop_type", "sg_file"])
        for year, dict_ct in output_sg_files_dict.items():
            output_ct_sg_files_dict = dict_ct
    ...
    with open(args.indices_output, 'w') as out_indices:  
        ...
        writer.writerow(["year", "crop_type", "sg_indices_file"])
        for year, dict_ct in output_indices_files_dict.items():
            output_ct_indices_files_dict = dict_ct
    ...
    with open(args.metrics_output, 'w') as out_metrics:  
        ...
        writer.writerow(["year", "crop_type", "sg_metrics_file"])
        for year, dict_ct in output_metrics_files_dict.items():
            output_ct_metrics_files_dict = dict_ct
