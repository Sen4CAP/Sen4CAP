...
class CropMaskProcessor(ProcessorBase):
    def create_context(self):
        ...
        group.add_argument(
            "-refp",
            help="The reference polygons",
            required=False,
            metavar="reference_polygons",
        )
        group.add_argument(
            "-refr",
            help="The reference raster",
            required=False,
            metavar="reference_raster",
        )
        parser.add_argument(
            "-ratio",
            help="The ratio between the validation and training polygons (default 0.75)",
            required=False,
            metavar="sample_ratio",
            default=0.75,
        )
        parser.add_argument(
            "-input",
            help="The list of products descriptors",
            required=True,
            metavar="product_descriptor",
            nargs="+",
        )
        parser.add_argument(
            "-trm",
            help="The temporal resampling mode (default resample)",
            choices=["resample", "gapfill"],
            required=False,
            default="resample",
        )
        parser.add_argument(
            "-classifier",
            help="The classifier (rf or svm) used for training (default rf)",
            required=False,
            metavar="classifier",
            choices=["rf", "svm"],
            default="rf",
        ...
        parser.add_argument(
            "-nbtrsample",
            help="The number of samples included in the training set (default 40000)",
            required=False,
            metavar="nbtrsample",
            default=40000,
        )
        parser.add_argument(
            "-rseed",
            help="The random seed used for training (default 0)",
            required=False,
            metavar="random_seed",
            default=0,
        )
        parser.add_argument(
            "-pixsize",
            help="The size, in meters, of a pixel (default 10)",
            required=False,
            metavar="pixsize",
            default=10,
        )
        parser.add_argument(
            "-red-edge",
            help="Include Sentinel-2 vegetation red edge bands",
            required=False,
            dest="red_edge",
            action="store_true",
        )
        parser.add_argument(
            "-no-red-edge",
            help="Don't include Sentinel-2 vegetation red edge bands",
            required=False,
            dest="red_edge",
            action="store_false",
        )
        ...
        parser.add_argument(
            "-sp",
            help="Per-sensor sampling rates (default SENTINEL 10 SPOT 5 LANDSAT 16)",
            required=False,
            nargs="+",
            default=["SENTINEL", "10", "SPOT", "5", "LANDSAT", "16"],
        )
        ...
        parser.add_argument(
            "-targetfolder",
            help="The folder where the target product is built",
            default="",
        )

        parser.add_argument(
            "-rfnbtrees",
            help="The number of trees used for training (default 100)",
            required=False,
            metavar="rfnbtrees",
            default=100,
        )
        parser.add_argument(
            "-rfmax",
            help="maximum depth of the trees used for Random Forest classifier (default 25)",
            required=False,
            metavar="rfmax",
            default=25,
        )
        parser.add_argument(
            "-rfmin",
            help="minimum number of samples in each node used by the classifier (default 25)",
            required=False,
            metavar="rfmin",
            default=25,
        )
        ...
        parser.add_argument(
            "-bm",
            help="Use benchmarking (vs. ATBD) features (default False)",
            required=False,
            type=bool,
            default=False,
        )
        parser.add_argument(
            "-window",
            help="The window, expressed in number of records, used for the temporal features extraction (default 6)",
            required=False,
            type=int,
            default=6,
        )
        parser.add_argument(
            "-crop-classes",
            help="List of crop classes (default 10, 11, 20)",
            required=False,
            type=int,
            nargs="+",
            default=[10, 11, 20],
        ...
        parser.add_argument(
            "-lmbd",
            help="The lambda parameter used in data smoothing (default 2)",
            required=False,
            metavar="lmbd",
            default=2,
        )
        parser.add_argument(
            "-eroderad",
            help="The radius used for erosion (default 1)",
            required=False,
            metavar="erode_radius",
            default="1",
        )
        parser.add_argument(
            "-alpha",
            help="The parameter alpha used by the mahalanobis function (default 0.01)",
            required=False,
            metavar="alpha",
            default="0.01",
        )
        ...
        parser.add_argument(
            "-nbcomp",
            help="The number of components used for dimensionality reduction (default 6)",
            required=False,
            type=int,
            default=6,
        )
        parser.add_argument(
            "-spatialr",
            help="The spatial radius of the neighborhood used for segmentation (default 10)",
            required=False,
            default=10,
        )
        parser.add_argument(
            "-ranger",
            help="The range radius defining the radius (expressed in radiometry unit) in the multispectral space (default 0.65)",
            required=False,
            default=0.65,
        )
        parser.add_argument(
            "-minsize",
            help="Minimum size of a region (in pixel unit) for segmentation (default 10)",
            required=False,
            default=10,
        )
        parser.add_argument(
            "-minarea",
            help="The minium number of pixel in an area where, for an equal number of crop and nocrop samples, the crop decision is taken (default 20)",
            required=False,
            default=20,
        )
        parser.add_argument(
            "-main-mission-segmentation",
            help="Only use main mission products for the segmentation",
            required=False,
            type=bool,
            default=True,
        )
        ...
        parser.add_argument(
            "-lut",
            help="Color LUT for previews (see /usr/share/sen2agri/crop-mask.lut)",
            required=False,
        )
        parser.add_argument("-outprops", help="Output properties file", required=False)

        parser.add_argument("-strata", help="Shapefiles with polygons for the strata")
        parser.add_argument(
            "-mode",
        ...
        parser.add_argument(
            "-stratum-filter",
            help="The list of strata to use in training and classification",
            required=False,
            type=int,
            nargs="+",
            default=None,
        ...
        parser.add_argument(
            "-tile-filter",
            help="The list of tiles to apply the classification to",
            required=False,
            nargs="+",
            default=None,
        )
        parser.add_argument(
            "-skip-segmentation",
            help="Skip the segmentation step, creating the product with just the raw mask (default false)",
            default=False,
            action="store_true",
        )
        parser.add_argument(
            "-reuse-segmentation",
            help="Reuse the segmentation output if present on disk (default false)",
            default=False,
            action="store_true",
        )
        parser.add_argument(
            "-skip-quality-flags",
            help="Skip quality flags extraction (default false)",
            default=False,
            action="store_true",
        )
        parser.add_argument(
            "-force-validation",
            help="Force validation (default false)",
            default=False,
            action="store_true",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "-tile-threads-hint",
            help="Number of threads to use for a single tile (default 4)",
            required=False,
            type=int,
            default=4,
        )
    ...
    def prepare_tile_high_par(self, tile):
        if self.args.refr is not None:
            ...
            step_args = [
                "otbcli",
                "SpectralFeaturesExtraction",
                self.args.buildfolder,
                "-progress",
                "false",
                "-mission",
                self.args.mission.name,
                "-pixsize",
                self.args.pixsize,
                "-lambda",
                self.args.lmbd,
        ...
        if self.args.refp is not None:
            ...
            for tile in stratum.tiles:
                ...
                step_args = [
                    ...
                    "-out",
                    format_otb_filename(
                        tile_stratum_reference_trimmed, compression="DEFLATE"
                    ),
                    "int16",
                ]
            ...
            if self.args.classifier == "svm":
                ...
                for tile in stratum.tiles:
                    ...
                    step_args = [
                        ...
                        "-out",
                        format_otb_filename(
                            tile_stratum_spectral_features, compression="DEFLATE"
                        ),
                        "int16",
                    ]
    ...
    def classify_tile(self, tile):
        ...
        if len(models) == 0:
            print(
                "Skipping classification for tile {} due to stratum filter".format(
                    tile.id
                )
            )
            return
            ...
            run_step(
                Step(
                    ...
                    [
                        ...
                        "ID",
                        "-in",
                        self.args.filtered_strata,
                        "-im",
                        tile.reference_raster,
                        "-out",
                        format_otb_filename(tile_model_mask, compression="DEFLATE"),
                        "uint8",
                    ],
                )
            )
        ...
        step_args = [
            ...
            tile_crop_mask_uncompressed,
            "-out",
            format_otb_filename(tile_crop_mask_map, compression="DEFLATE"),
            "int16",
        ]
    ...
    def postprocess_tile(self, tile):
        ...
        if not os.path.exists(tile_crop_mask):
            print(
                "Skipping post-processing for tile {} due to missing raw mask".format(
                    tile.id
                )
            )
            return
        ...
        if not self.args.reuse_segmentation:
            ...
            needs_pca = (
                needs_tile_smoothed or needs_tile_smoothed_spatial
        ...
        step_args = [
            ...
            self.args.tmpfolder,
            "-out",
            format_otb_filename(tile_segmentation, compression="DEFLATE"),
            "uint32",
        ]
        ...
        step_args = [
            ...
            1024,
            "-out",
            format_otb_filename(tile_segmentation_merged, compression="DEFLATE"),
            "uint32",
        ]
        ...
        step_args = [
            ...
            "-rout",
            format_otb_filename(tile_segmented, compression="DEFLATE"),
            "int16",
        ]
    ...
    def validate(self, context):
        ...
        if want_validation:
            for stratum in self.strata:
                ...
                run_step(
                    Step(
                        "ComputeConfusionMatrix_" + str(stratum.id),
                        step_args,
                ...
                step_args = [
                    ...
                    area_statistics,
                    "-quality",
                    area_quality_metrics,
                    "-out",
                    area_validation_metrics_xml,
                ]
            ...
            if not self.single_stratum:
                ...
                if len(self.strata) > 1:
                    ...
                    shutil.copyfile(
                        area_validation_metrics_xml, global_validation_metrics_xml
                    )
    ...
    def get_lut_path(self):
        if self.args.lut is not None:
            ...
            if os.path.isfile(lut_path):
                ...
                print(
                    "Warning: The LUT file {} does not exist, using the default one".format(
                        self.args.lut
                    )
                )
        ...
        lut_path = os.path.join(
            script_dir, "../sen2agri-processors/CropMask/crop-mask.lut"
        )
    ...
    def build_metadata(self):
        ...
        for tile in self.tiles:
            ...
            tiles.append(E.Tile(E.Id(tile.id), inputs))

        metadata = E.Metadata(
            E.ProductType("Crop Mask"), E.Level("L4A"), E.SiteId(self.args.siteid)
        )

        if self.args.refp is not None:
            metadata.append(E.ReferencePolygons(os.path.basename(self.args.refp)))
        else:
            metadata.append(E.ReferenceData(os.path.basename(self.args.refr)))

        if self.args.strata is not None:
            metadata.append(E.Strata(os.path.basename(self.args.strata)))
        ...
        if self.args.lut is not None:
            parameters.append(E.LUT(os.path.basename(self.args.lut)))
        ...
        if self.args.refr is not None:
            parameters.append(
                E.Smoothing(
                    E.Window(str(self.args.window)), E.Lambda(str(self.args.lmbd))
                )
            )
            parameters.append(E.ErosionRadius(str(self.args.eroderad)))
            parameters.append(E.TrimmingAlpha(str(self.args.alpha)))

        classifier = E.Classifier(E.TrainingSamplesPerTile(str(self.args.nbtrsample)))
