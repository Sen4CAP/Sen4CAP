...
def handle_batch_record(selCols, all_cropfields, classifier, df_results_all):
    ...
    for cropfield_descr in all_cropfields:
        ...
        for renamed_col in selCols.dict_cols_indices.keys():
            ...
            i = 0
            for date in selCols.dict_cols_dates[renamed_col]:
...
def handle_ipc_file(input, out, classifier, df_results_all) :
...
def handle_csv_file(input, out, classifier, df_results_all) :
...
def handle_json_file(input, out, classifier, df_results_all) :
...
def handle_file(input, output, classifier):
    ...
    if lcinput.endswith('.ipc'):
        ...
        df_results_all = handle_ipc_file(input, output, classifier, df_results_all)
    elif lcinput.endswith('.csv'):
        print("Handling csv file {}".format(input))
        df_results_all = handle_csv_file(input, output, classifier, df_results_all)
    elif lcinput.endswith('.json'):
        print("Handling json file {}".format(input))
        df_results_all = handle_json_file(input, output, classifier, df_results_all)        
    else :
