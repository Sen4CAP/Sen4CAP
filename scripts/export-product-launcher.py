...
def get_product_id_by_full_path(conn, prd_full_path):
    ...
    with conn.cursor() as cursor:
        query = SQL(""" select id from product where full_path = {} """)
...
def create_prd_shp_files(vectDataDirPath):
    ...
    for f in glob(os.path.join(vectDataDirPath, "*.gpkg")):
        ...
        ogr2ogr_cmd = ["ogr2ogr"]
        ogr2ogr_cmd += [
            os.path.join(vectDataDirPath, gpkgFileName + ".shp"),
...
def main():
    ...
    parser.add_argument("-g", "--ogr2ogr-path", default="ogr2ogr", help="Unused")
    ...
    for line in reader:
        if len(line) > 0:
            ...
            print(
                "Error reading product id and path from the file {}".format(
                    args.product_ids_file
                )
            )
            exit(1)
