...
# imG_path = '/scratch/diheymans/saxony'
# path_out = '/export/homes/diheymans/SEN4CAP/2022_UseCases/01_SpatialNonCongruency/04_ProdV2/saxony/'
# site = 'saxony'
# tile='33UVS'
# year = 2021

# band_list = ['B2','B3','B4','B8','NDVI']

# Period_l = range(1,12) #list of all the period of monitoring (here months)
# n = 3 #number of images to use in the period (if month: 3 images of 10 days resampled S2)

# L2A_image = glob.glob(f'{imG_path}/S2_Resampling_*_2021_B8_noData.tif')
# img_ds = gdal.Open(L2A_image[0],gdal.GA_ReadOnly)

# raster_5mBuffer = f'/export/homes/diheymans/SEN4CAP/2022_UseCases/01_SpatialNonCongruency/03_Insitu/04_Saxon/lpis/decl_{site}_{year}_{tile}_S2.tif'
...
#       TODO : these two will be in the calls outside, from the orchestrator
...
#       TODO : these two will be called outside, from the orchestrator
# def remove_isolated(file_in, file_out, noDataClassif, min_nb, radius,list_class):
#     '''
#     min nb = minimum neighbors (1 pixel by default) to be replaced
#     radius = radius of search (1 pixel by default)
#     nan = no-data (0 by default)
#     '''
#     
#     cmd = '/usr/local/engebin/neighbour/removeIsolated'
#     cmd += f' {file_out}'
#     cmd += f' {file_in}'
#     cmd += f' {min_nb}'
#     cmd += f' {radius}'
...
# 
# 
# def localConnectivity(file_out_post1, file_out_post,radiusC):
#     
#     cmd = '/export/homes/tom/git/code/otb/neighbour/build/localClassConnectivityIndex'
...
def do_clustering(input_images, lpis_buffered_raster, period, num_imgs, num_clusters, out_file_cluster) :
    img_ds = gdal.Open(input_images[0],gdal.GA_ReadOnly)
    ...
    # print("date_l = {}".format(len(date_l)))
