...
def copyConfigFile(config, conn, site_id, config_file):
    ...
    if cfgDir == "":
        ...
        setConfigValue(
            conn,
            None,
            "processor.s4c_l4b.cfg_dir",
            "/mnt/archive/grassland_mowing/{site}/{year}/config/",
        )
    ...
    if cfgYear == "":
        # add the general key
        print(
            "The processor.s4c_l4b.year key is not configured in the database. Creating it ..."
        )
...
def main():
    ...
    parser.add_argument(
        "-d", "--mowing-start-date", required=False, help="The mowing start date"
    )
