...
class CropTypeProcessor(ProcessorBase):
    def create_context(self):
        ...
        parser.add_argument(
            "-nbtrsample",
            help="The number of samples included in the training set (default 10000)",
            required=False,
            metavar="nbtrsample",
            default=10000,
        )
        ...
        parser.add_argument(
            "-mask",
            help="The crop mask for each tile",
            required=False,
            nargs="+",
            default=None,
        )
        parser.add_argument(
            "-maskprod",
            help="A crop mask product for the same tiles",
            required=False,
            default=None,
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "-lut",
            help="Color LUT for previews (see /usr/share/sen2agri/crop-type.lut)",
            required=False,
        )
        ...
        parser.add_argument(
            "-include-raw-map",
            help="Include the unmasked crop type map even when a crop mask was used (default true)",
            required=False,
            dest="include_raw_map",
            action="store_true",
        )
        parser.add_argument(
            "-no-include-raw-map",
            help="Don't include the unmasked crop type map when a crop mask was used",
            required=False,
            dest="include_raw_map",
            action="store_false",
        )
        ...
        parser.add_argument(
            "-tile-threads-hint",
            help="Number of threads to use for a single tile, except for the post-filtering step (default 4)",
            required=False,
            type=int,
            default=4,
        )
            ...
            for tile_dir in os.listdir(tile_path):
                ...
                if m:
                    ...
                    img_data_path = os.path.join(
                        os.path.join(tile_path, tile_dir), "IMG_DATA"
                    )
    ...
    def classify_tile(self, tile):
        ...
        tile_crop_type_map_uncompressed = self.get_output_path(
            "crop_type_map_{}_uncompressed.tif", tile.id
        )

        step_args = [
            "otbcli",
            "CropTypeImageClassifier",
            self.args.buildfolder,
            "-progress",
            "false",
            "-mission",
        ...
        step_args = [
            ...
            tile_crop_type_map_uncompressed,
            "-out",
            format_otb_filename(tile_crop_type_map, compression="DEFLATE"),
            "int16",
        ]
    ...
    def postprocess_tile(self, tile):
        if tile.crop_mask is not None:
            ...
            step_args = [
                ...
                tile.crop_mask,
                "-out",
                format_otb_filename(tile_crop_map_masked, compression="DEFLATE"),
                "int16",
            ]
    ...
    def get_lut_path(self):
        ...
        lut_path = os.path.join(
            script_dir, "../sen2agri-processors/CropType/crop-type.lut"
        )
    ...
    def build_metadata(self):
        ...
        for tile in self.tiles:
            ...
            tile_el = E.Tile(E.Id(tile.id), inputs)
            if tile.crop_mask is not None:
                tile_el.append(
