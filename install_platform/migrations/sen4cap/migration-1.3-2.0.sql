    ...
    if exists (select * from information_schema.tables where table_schema = 'public' and table_name = 'meta') then
        if exists (select * from meta where version in ('1.3', '2.0')) then
            ...
            _statement := $str$
                ...
                INSERT INTO config(key, site_id, value, last_updated) VALUES ('general.orchestrator.docker_image', NULL, 'sen4cap/processors:2.0.0', '2021-01-14 12:11:21.800537+00') ON conflict DO nothing;
