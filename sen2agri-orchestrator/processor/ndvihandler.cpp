                                             ...
                                             const JobSubmittedEvent &event)
{
    ...
    const ProductList &prds = GetInputProducts(ctx, parameters, configParameters, event.siteId,
                                               ProductType::L2AProductTypeId, "processor.l3b.");
...
QStringList NdviHandler::GetNdviProductFormatterArgs(TaskToSubmit &productFormatterTask, EventProcessingContext &ctx, const JobSubmittedEvent &event,
    ...
    for(const TileInfos &tileInfo: prdTilesInfosList) {
        ...
        if (tileIds.size() == 0) {
            Logger::error(QStringLiteral("GetNdviProductFormatterArgs: cannot extract tile id from file %1!")
...
bool NdviHandler::FilterTile(const QSet<QString> &tilesSet, const ProductDetails &prdDetail)
{
    const QStringList &tileIds = prdDetail.GetProduct().tiles;
    if (tileIds.size() == 0) {
        Logger::error(QStringLiteral("FilterTile: cannot extract tile id from product %1!")
                      .arg(prdDetail.GetProduct().fullPath));
        return false;
    }
